...
def reapply_routing() -> None:
    """Повторно применить outbound + routing для Relay (после rebuild_configs)."""
    from vwn.core import config as _cfg
    mode = _cfg.vwn_conf_get("RELAY_TUNNEL_MODE") or ""
    if not mode or not os.path.isfile(CONFIG):
        return
    with open(CONFIG) as f:
        saved = json.load(f)
    ob = _build_outbound(saved)
    _apply_outbound(ob)
